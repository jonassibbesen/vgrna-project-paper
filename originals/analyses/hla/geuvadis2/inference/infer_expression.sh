...
# Map reads
/usr/bin/time -v bash -c "vg mpmap -t ${CPU} -n rna -x ${GRAPH}.xg -g ${GRAPH}_index_up_hla.gcsa -d ${GRAPH}_index.dist -f reads_1.fq.gz -f reads_2.fq.gz > alignments.gamp"
...
# Infer expression
/usr/bin/time -v bash -c "/rpvg/bin/rpvg -t ${CPU} -r ${SEED} -i haplotype-transcripts -g ${GRAPH}.xg -p ${TRANSCRIPTS}_6.gbwt -a alignments.gamp -f transcript_info.txt -o ${OUT_PREFIX}"

# Compress expression values
/usr/bin/time -v bash -c "gzip ${OUT_PREFIX}.txt; gzip ${OUT_PREFIX}_haps.txt"

# Upload expression 
aws s3 sync . s3://vg-k8s/users/jsibbesen/vgrna/analyses/hla/geuvadis2/inference/${NAME}/${TRANSCRIPTS}/ --exclude "*" --include "${OUT_PREFIX}*" --no-progress
