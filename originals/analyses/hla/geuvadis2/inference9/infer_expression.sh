...
# Infer expression
/usr/bin/time -v bash -c "/rpvg/bin/rpvg -t ${CPU} -r ${SEED} --filt-best-score 0.95 --ind-hap-inference -i haplotype-transcripts -g ${GRAPH}.xg -p ${TRANSCRIPTS}_6.gbwt -a alignments.gamp -f transcript_info.txt -o ${OUT_PREFIX}"

# Compress expression values
/usr/bin/time -v bash -c "gzip ${OUT_PREFIX}.txt; gzip ${OUT_PREFIX}_haps.txt"

# Upload expression 
aws s3 sync . s3://vg-k8s/users/jsibbesen/vgrna/analyses/hla/geuvadis2/inference9/${NAME}/${TRANSCRIPTS}/ --exclude "*" --include "${OUT_PREFIX}*" --no-progress
