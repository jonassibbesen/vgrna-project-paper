...
OUT_PREFIX="rpvg_mpmap_r2_${NAME}_${TRANSCRIPTS}"
...
for READ in $(echo ${READS_1} | sed -e 's/,/ /g'); do 
	...
	# Download reads
	IDX=$((IDX+1))
	wget --no-check-certificate --no-verbose -t 10 -O reads_${IDX}_1.fq.gz ${READ}
done
...
for READ in $(echo ${READS_2} | sed -e 's/,/ /g'); do 
	...
	# Download reads
	IDX=$((IDX+1))
	wget --no-check-certificate --no-verbose -t 10 -O reads_${IDX}_2.fq.gz ${READ}
done
...
# Infer expression
/usr/bin/time -v bash -c "rpvg -t ${CPU} -r ${SEED} -e rf --max-num-sd-frag 5 --max-score-diff 10 --filt-best-score 0.95 -n 1000 -i haplotype-transcripts -g ${GRAPH}.xg -p ${TRANSCRIPTS}_6.gbwt -a alignments.gamp -f transcript_info.txt -o ${OUT_PREFIX}"

# Compress expression values
/usr/bin/time -v bash -c "gzip ${OUT_PREFIX}.txt; gzip ${OUT_PREFIX}_joint.txt"

# Upload expression 
aws s3 sync . s3://vg-k8s/users/jsibbesen/vgrna/analyses/hla/hgsvc/inference_r2/${NAME}/${TRANSCRIPTS}/ --exclude "*" --include "${OUT_PREFIX}*" --no-progress
