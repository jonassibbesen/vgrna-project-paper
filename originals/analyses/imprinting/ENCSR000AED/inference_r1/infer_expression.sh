...
ALIGN_PREFIX="mpmap_${GRAPH}_real_r1_${NAME}"
OUT_PREFIX="rpvg_mpmap_r1_strand_${NAME}_${TRANSCRIPTS}"

# Download alignments
aws s3 cp s3://vg-k8s/users/jsibbesen/vgrna/benchmark/whole_genome/mapping/vg/alignments/polya_rna/real_r1/${NAME}/mpmap/${GRAPH}/${ALIGN_PREFIX}.gamp . --no-progress
...
# Upload expression 
aws s3 sync . s3://vg-k8s/users/jsibbesen/vgrna/analyses/imprinting/ENCSR000AED/inference_r1/${NAME}/${TRANSCRIPTS}/ --exclude "*" --include "${OUT_PREFIX}*" --no-progress
