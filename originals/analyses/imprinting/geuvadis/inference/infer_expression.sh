...
for READ in $(echo ${READS_1} | sed -e 's/,/ /g'); do 
	...
	# Download reads
	IDX=$((IDX+1))
	wget --no-check-certificate --no-verbose -O reads_${IDX}_1.fq.gz ${READ}
done
...
for READ in $(echo ${READS_2} | sed -e 's/,/ /g'); do 
	...
	# Download reads
	IDX=$((IDX+1))
	wget --no-check-certificate --no-verbose -O reads_${IDX}_2.fq.gz ${READ}
done
...
# Infer expression
/usr/bin/time -v bash -c "/rpvg/bin/rpvg -t ${CPU} -r ${SEED} -n 1 -i haplotype-transcripts -g ${GRAPH}.xg -p ${TRANSCRIPTS}.gbwt -a alignments.gamp -f transcript_info.txt -o ${OUT_PREFIX}"

# Compress expression values
/usr/bin/time -v bash -c "gzip ${OUT_PREFIX}.txt; gzip ${OUT_PREFIX}_haps.txt"

# Upload expression 
aws s3 sync . s3://vg-k8s/users/jsibbesen/vgrna/analyses/imprinting/geuvadis/inference/${NAME}/${TRANSCRIPTS}/ --exclude "*" --include "${OUT_PREFIX}*" --no-progress
