...
/usr/bin/time -v bash -c "vg index -p -t ${CPU} -G ${CHR}.gbwt -v ${EXONVARS_PREFIX}_${CHR}.vcf.gz ${OUT_PREFIX}.pg"
...
/usr/bin/time -v bash -c "vg rna -p -t ${CPU} -o -c -r -a -u -g -n ${CHR}.gtf -l ${CHR}.gbwt -b ${OUT_PREFIX}.gbwt -f ${OUT_PREFIX}.fa -i ${OUT_PREFIX}.txt ${OUT_PREFIX}.pg > ${OUT_PREFIX}_tmp.pg; mv ${OUT_PREFIX}_tmp.pg ${OUT_PREFIX}.pg"
