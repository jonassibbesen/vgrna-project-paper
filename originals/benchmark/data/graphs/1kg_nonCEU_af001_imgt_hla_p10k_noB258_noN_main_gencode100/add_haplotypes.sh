...
aws s3 cp s3://vg-k8s/users/jsibbesen/vgrna/data/haplotypes/imgt_hla/A/A_cds_alleles_full_pad10k_noB258.fa . --no-progress
aws s3 cp s3://vg-k8s/users/jsibbesen/vgrna/data/haplotypes/imgt_hla/B/B_cds_alleles_full_pad10k_noB258.fa . --no-progress
aws s3 cp s3://vg-k8s/users/jsibbesen/vgrna/data/haplotypes/imgt_hla/C/C_cds_alleles_full_pad10k_noB258.fa . --no-progress
aws s3 cp s3://vg-k8s/users/jsibbesen/vgrna/data/haplotypes/imgt_hla/DQB1/DQB1_cds_alleles_full_pad10k_noB258.fa . --no-progress
aws s3 cp s3://vg-k8s/users/jsibbesen/vgrna/data/haplotypes/imgt_hla/DRB1/DRB1_cds_alleles_full_pad10k_noB258.fa . --no-progress

# Filter HLA CDS null alleles
/usr/bin/time -v bash -c "wc -l *_cds_alleles_full_pad10k_noB258.fa; grep '>' *_cds_alleles_full_pad10k_noB258.fa | wc -l; grep -h '>' *_cds_alleles_full_pad10k_noB258.fa | grep -v 'N$' | sed -e 's/>//g' > no_null_alleles.txt; wc -l no_null_alleles.txt; seqtk subseq <(cat *_cds_alleles_full_pad10k_noB258.fa) no_null_alleles.txt > cds_alleles_full_pad10k_noB258_non.fa; wc -l cds_alleles_full_pad10k_noB258_non.fa; grep '>' cds_alleles_full_pad10k_noB258_non.fa | wc -l"

# Map HLA CDS alleles
/usr/bin/time -v bash -c "vg mpmap -t ${CPU} -l long -F GAM -x base.xg -g index.gcsa -d index.dist -f cds_alleles_full_pad10k_noB258_non.fa > cds.gam"
