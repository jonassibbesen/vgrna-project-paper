...
if [ "${CHR}" = "SCA" ]; then
	...
	/usr/bin/time -v bash -c "vg rna -p -t ${CPU} -e -d -o -r -n ${CHR}.gtf ${OUT_PREFIX}.pg > ${OUT_PREFIX}_tmp.pg; mv ${OUT_PREFIX}_tmp.pg ${OUT_PREFIX}.pg"
...
# Chromosome is canonical
	...
	# Create haplotype-specific transcripts and update graph
	/usr/bin/time -v bash -c "vg rna -p -t ${CPU} -e -d -o -r -g -u -n ${CHR}.gtf -b ${OUT_PREFIX}.gbwt -f ${OUT_PREFIX}.fa -i ${OUT_PREFIX}.txt ${OUT_PREFIX}.pg > ${OUT_PREFIX}_tmp.pg; mv ${OUT_PREFIX}_tmp.pg ${OUT_PREFIX}.pg"
