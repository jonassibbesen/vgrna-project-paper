...
OUT_PREFIX="sim_${TRANSCRIPTS_PREFIX}_${EXPRESSION_PREFIX}_vg_r1_h2"
...
aws s3 sync . s3://vg-k8s/users/jsibbesen/vgrna/benchmark/whole_genome/data/reads/sim/${TRANSCRIPTS_PREFIX}/${EXPRESSION_PREFIX}/vg_r1/ --exclude "*" --include "${OUT_PREFIX}*" --exclude "${OUT_PREFIX}.fq" --exclude "${OUT_PREFIX}.gam" --no-progress
