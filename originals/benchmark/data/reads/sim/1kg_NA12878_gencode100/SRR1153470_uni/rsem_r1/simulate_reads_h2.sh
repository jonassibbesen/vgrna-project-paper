...
OUT_PREFIX="sim_${PROFILE_PREFIX}_rsem_r1_h2"
...
/usr/bin/time -v bash -c "zcat */*.txt.gz | grep NA12878 | grep -v _0_ | cut -f1 | grep -v Name > haplotype_transcripts.txt; seqtk subseq <(zcat */*fa.gz) haplotype_transcripts.txt > ${TRANSCRIPTS_PREFIX}.fa; grep '"'>'"' ${TRANSCRIPTS_PREFIX}.fa | wc -l"
