...
# Create CEU samples file
/usr/bin/time -v bash -c "grep CEU ${VARSINFO_PREFIX}.panel | grep -v sample | cut -f1 > samples.txt; wc -l samples.txt"
...
# Select CEU samples and exonic variants
/usr/bin/time -v bash -c "bcftools annotate -x ^INFO/AC,^INFO/AF,^INFO/AN ${ALLVARS_PREFIX}.vcf.gz | bcftools norm -m -any -f ${GENOME_PREFIX}.fa | bcftools view -S samples.txt -c 1 | bcftools norm -m +any -O z -f ${GENOME_PREFIX}.fa > ${OUT_PREFIX}.vcf.gz; tabix ${OUT_PREFIX}.vcf.gz"
