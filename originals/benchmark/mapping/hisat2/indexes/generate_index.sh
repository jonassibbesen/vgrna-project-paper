...
if [ "${VARIANTS}" = "" ]; then
	...
	/usr/bin/time -v bash -c "hisat2-build -p ${CPU} --exon exons.txt --ss splice_sites.txt ${GENOME_PREFIX}.fa ${OUT_PREFIX}"
	...
	# Construct HISAT2 index
	/usr/bin/time -v bash -c "hisat2-build -p ${CPU} --snp variants.snp --haplotype variants.haplotype --exon exons.txt --ss splice_sites.txt ${GENOME_PREFIX}.fa ${OUT_PREFIX}"
fi
