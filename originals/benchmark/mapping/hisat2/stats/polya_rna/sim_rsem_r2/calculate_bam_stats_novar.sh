...
ALIGN_PREFIX="${MAPPER}_${REF}_sim_rsem_r2_${SIM}"
SIM_PREFIX="sim_1kg_NA12878_gencode100_${SIM}_rsem_r1"
OUT_PREFIX="${ALIGN_PREFIX}"

# Download alignments
aws s3 cp s3://vg-k8s/users/jsibbesen/vgrna/benchmark/whole_genome/mapping/hisat2/alignments/polya_rna/sim_rsem_r2/${SIM}/${MAPPER}/${REF}/ . --recursive --exclude "*" --include "*.bam" --include "*.bam.bai" --no-progress
...
for i in $(seq 1 2); do
	...
	/usr/bin/time -v bash -c "convert_rsem_sim_info ${ALIGN_PREFIX}_h${i}.bam ${SIM_PREFIX}_h${i}.sim.isoforms.results > ${ALIGN_PREFIX}_h${i}_pos.txt; wc -l ${ALIGN_PREFIX}_h${i}_pos.txt"
	...
	/usr/bin/time -v bash -c "calc_vg_benchmark_stats ${ALIGN_PREFIX}_h${i}_rsort.bam ${TRANSCRIPTS}.bam ${ALIGN_PREFIX}_h${i}_pos.txt 3 transcripts.gff3 null null null > ${OUT_PREFIX}_bam_ovl3_h${i}.txt; gzip ${OUT_PREFIX}_bam_ovl3_h${i}.txt"
done
...
# Upload statistics 
aws s3 sync . s3://vg-k8s/users/jsibbesen/vgrna/benchmark/whole_genome/mapping/hisat2/stats/polya_rna/sim_rsem_r2/${SIM}/${MAPPER}/${REF}/ --exclude "*" --include "${OUT_PREFIX}*txt.gz" --no-progress
