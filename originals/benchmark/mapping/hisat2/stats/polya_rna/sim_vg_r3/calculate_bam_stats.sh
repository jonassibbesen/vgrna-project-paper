...
OUT_PREFIX="${MAPPER}_${REF}_sim_vg_r3_${SIM}"

# Download alignments
aws s3 cp s3://vg-k8s/users/jsibbesen/vgrna/benchmark/whole_genome/mapping/hisat2/alignments/polya_rna/sim_vg_r2/${SIM}/${MAPPER}/${REF}/ . --recursive --exclude "*" --include "*.bam" --include "*.bam.bai" --no-progress
...
for i in $(seq 1 2); do
	...
	/usr/bin/time -v bash -c "calc_vg_benchmark_stats ${ALIGN_PREFIX}_h${i}_rsort.bam ${TRANSCRIPTS}.bam <(zcat ${SIM_PREFIX}_h${i}.txt.gz) 3 transcripts.gff3 null 1kg_NA12878_exons.vcf.gz NA12878 <(zcat ${SIM_PREFIX}_h${i}_edit_dist.txt.gz) > ${OUT_PREFIX}_bam_ovl3_h${i}.txt; gzip ${OUT_PREFIX}_bam_ovl3_h${i}.txt"
done

# Upload statistics 
aws s3 sync . s3://vg-k8s/users/jsibbesen/vgrna/benchmark/whole_genome/mapping/hisat2/stats/polya_rna/sim_vg_r3/${SIM}/${MAPPER}/${REF}/ --exclude "*" --include "${OUT_PREFIX}*txt.gz" --no-progress
