...
for i in $(seq 1 2); do
	...
	# Inject bam to gam
	/usr/bin/time -v bash -c "vg inject -t ${CPU} -x ${GRAPH}.xg ${OUT_PREFIX}_h${i}_primary.bam | vg view -a - | sed 's/\/1\",/\",/g' | sed 's/_1\/2\",/_2\",/g' | vg view -a -G -J - > ${OUT_PREFIX}_h${i}.gam"
done 

# Upload statistics 
aws s3 sync . s3://vg-k8s/users/jsibbesen/vgrna/benchmark/whole_genome/mapping/star/alignments/polya_rna/sim_vg_r1/${SIM}/${MAPPER}/${REF}/ --exclude "*" --include "${OUT_PREFIX}*.gam" --no-progress
