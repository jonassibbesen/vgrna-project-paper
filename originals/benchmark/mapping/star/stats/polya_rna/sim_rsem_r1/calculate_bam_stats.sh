...
# Download alignments
aws s3 cp s3://vg-k8s/users/jsibbesen/vgrna/benchmark/whole_genome/mapping/star/alignments/polya_rna/sim_rsem_r1/${SIM}/${MAPPER}/${REF}/ . --recursive --exclude "*" --include "*.bam" --include "*.bam.bai" --no-progress
...
for i in $(seq 1 2); do
	...
	/usr/bin/time -v bash -c "convert_rsem_sim_info ${ALIGN_PREFIX}_h${i}_fixed.bam ${SIM_PREFIX}_h${i}.sim.isoforms.results > ${ALIGN_PREFIX}_h${i}_pos.txt"

	# Calculate overlap statistics
	/usr/bin/time -v bash -c "calc_vg_benchmark_stats ${ALIGN_PREFIX}_h${i}_fixed.bam ${TRANSCRIPTS}.bam ${ALIGN_PREFIX}_h${i}_pos.txt 3 1kg_NA12878_exons.vcf.gz NA12878 > ${OUT_PREFIX}_ovl3_vg_h${i}.txt; gzip ${OUT_PREFIX}_ovl3_vg_h${i}.txt"
done
...
# Upload statistics 
aws s3 sync . s3://vg-k8s/users/jsibbesen/vgrna/benchmark/whole_genome/mapping/star/stats/polya_rna/sim_rsem_r1/${SIM}/${MAPPER}/${REF}/ --exclude "*" --include "${OUT_PREFIX}*txt.gz" --no-progress
