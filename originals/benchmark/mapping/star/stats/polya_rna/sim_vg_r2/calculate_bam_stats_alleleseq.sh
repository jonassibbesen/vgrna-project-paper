...
ALIGN_PREFIX="star_alleleseq_diploid_gencode100_sim_vg_r1"
SIM_PREFIX="sim_1kg_NA12878_gencode100_${SIM}_vg_r1"
OUT_PREFIX="${MAPPER}_${REF}_sim_vg_r2_${SIM}"

# Download alignments
aws s3 cp s3://vg-k8s/users/jeizenga/alleleseq/ . --recursive --exclude "*" --include "${ALIGN_PREFIX}*${SIM}*${READS_1}*.bam" --no-progress
aws s3 cp s3://vg-k8s/users/jeizenga/alleleseq/ . --recursive --exclude "*" --include "${ALIGN_PREFIX}*${SIM}*${READS_2}*.bam" --no-progress
...
if [ "${REF}" = "1kg_NA12878_gencode80" ]; then 
...
for i in $(seq 1 2); do
	...
	/usr/bin/time -v bash -c "calc_vg_benchmark_stats ${ALIGN_PREFIX}_h${i}_rsort.bam ${TRANSCRIPTS}.bam <(zcat ${SIM_PREFIX}_h${i}.txt.gz) 3 transcripts.gff3 null 1kg_NA12878_exons.vcf.gz NA12878 > ${OUT_PREFIX}_bam_ovl3_h${i}.txt; gzip ${OUT_PREFIX}_bam_ovl3_h${i}.txt"
done
