...
OUT_PREFIX="${MAPPER}_${REF}_sim_rsem_r2_${SIM}"
...
for i in $(seq 1 2); do
	...
	elif [ "${MAPPER}" = "map_fast_multi10" ]; then

		# Map reads
		/usr/bin/time -v bash -c "vg map -t ${CPU} --try-up-to 16 --mate-rescues 32 -M 10 -x ${REF}.xg -g ${REF}_index.gcsa -f ${READS_PREFIX}_h${i}_1.fq.gz -f ${READS_PREFIX}_h${i}_2.fq.gz > ${OUT_PREFIX}_h${i}.gam"
	...
	# Use default vg mpmap with multimapping
		...
		# Map reads
		/usr/bin/time -v bash -c "vg mpmap -t ${CPU} -n rna -M 10 -U -x ${REF}.xg -g ${REF}_index.gcsa -d ${REF}_index.dist -f ${READS_PREFIX}_h${i}_1.fq.gz -f ${READS_PREFIX}_h${i}_2.fq.gz > ${OUT_PREFIX}_h${i}.gamp"
	fi
done

# Upload read alignments 
aws s3 sync . s3://vg-k8s/users/jsibbesen/vgrna/benchmark/whole_genome/mapping/vg/alignments/polya_rna/sim_rsem_r2/${SIM}/${MAPPER}/${REF}/ --exclude "*" --include "${OUT_PREFIX}*" --no-progress
