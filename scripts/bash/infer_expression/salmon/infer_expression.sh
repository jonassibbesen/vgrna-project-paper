...
# Set index directory
INDEX_DIR="salmon_index_1kg_nonCEU_af001_gencode100"
...
if [ "${MODE}" = "salmon" ]; then
	...
	# Infer expression
	/usr/bin/time -v bash -c "salmon quant -p ${CPU} -l A -i ${INDEX_DIR} -o ${OUT_PREFIX} -1 ${READ_1} -2 ${READ_2}"

# Use Salmon with bias correction
elif [ "${MODE}" = "salmon_bias" ]; then

	# Infer expression
	/usr/bin/time -v bash -c "salmon quant -p ${CPU} -l A --seqBias --gcBias -i ${INDEX_DIR} -o ${OUT_PREFIX} -1 ${READ_1} -2 ${READ_2}"
fi
