...
OUT_PREFIX="sim_1kg_NA12878_gencode100_ENCSR000AED_rep1_uni_vg_h2"

# Set number of threads
CPU=1

# Extract haplotype-specific expression
/usr/bin/time -v bash -c "zcat ${TRANSCRIPTS_PREFIX}.txt.gz | grep NA12878 | grep -v _0_ | cut -f1 | grep -v Name > haplotype_transcripts.txt; echo transcript_id >> haplotype_transcripts.txt; wc -l haplotype_transcripts.txt; grep -F -f haplotype_transcripts.txt ${PROFILE_PREFIX}.isoforms.results > expression_haplotype.isoforms.results; wc -l expression_haplotype.isoforms.results; rm haplotype_transcripts.txt"

# Subset read pairs
/usr/bin/time -v bash -c "seqtk sample -s ${SEED} ENCFF001REK.fastq.gz 10000000 > subset_1.fq; seqtk sample -s ${SEED} ENCFF001REJ.fastq.gz 10000000 > subset_2.fq; wc -l subset_1.fq; wc -l subset_2.fq"
